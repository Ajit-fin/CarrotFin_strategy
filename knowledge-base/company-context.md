...
CarrotFin is an **AI-native household financial intelligence system** for Indian consumers. Not a robo-advisor. Not a dashboard with a chatbot bolted on. The AI doesn't assist — it *runs* the experience. The atomic unit is the **household/family** (Data-Model, GP-01), not the individual — because in India, financial decisions are inherently collective. Every screen, every recommendation, every interaction is generated by the AI based on who the household is, what they need right now, and what they should do next.

The product integrates conversational AI advisory with adaptive, generative UI. The AI composes the experience — deciding what to show, in what form, and at what depth — based on user context. The specific surface architecture (how many surfaces, their roles, and how users navigate) evolves as a design decision informed by user data, not as a fixed principle.
...
- **Not a static dashboard** — every existing Indian finance app (ET Money, INDmoney, Kuvera) shows the same layout to everyone. CarrotFin's AI composes a different interface for every user based on their context, needs, and behavioral mode.
